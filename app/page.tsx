...
export default function Page() {
  ...
  // Auto-initialize assignments on app startup (for Vercel deployments)
  useEffect(() => {
    const initializeApp = async () => {
      try {
        const { autoInitializeAssignments } = await import('../data/mockData');
        await autoInitializeAssignments();
        console.log('🎄 App initialized successfully for production');
      } catch (error) {
        console.warn('⚠️ App initialization had issues, will work on-demand:', error);
      } finally {
